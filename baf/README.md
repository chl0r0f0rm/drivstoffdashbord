...
Automatisk innhenting via GitHub Actions → `data/baf_data.csv` → Power Automate → SharePoint → Power BI.
...
| `baf_csv.py` | Les/skriv `data/baf_data.csv` (kolonnekontrakt for Power BI) |
...
GitHub Actions (dag 15 + siste dag i mnd, 04:00 UTC)
  → fetch_baf.py
  → data/baf_data.csv (git-snapshot)
